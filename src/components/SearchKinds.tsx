...
const Movie = () => {
  ...
  useEffect(() => {
    dispatch(searchMovie({ title: queryTitle }));
  }, [dispatch, queryTitle]);

  // 2022/12/16 - 현재 검색된 영화와 유사 영화들 검색 - by 1-blue
  useEffect(() => {
    if (!searchedMovies || searchedMovies.results.length === 0) return;

    dispatch(similarMovie({ movieId: searchedMovies.results[0].id }));
  ...
  const target = useMemo(() => {
    if (!searchedMovies || searchedMovies.results.length === 0) return null;

    return searchedMovies.results[0];
  }, [searchedMovies]);
...
const Drama = () => {
  ...
  useEffect(() => {
    dispatch(searchDrama({ title: queryTitle }));
  }, [dispatch, queryTitle]);

  // 2022/12/16 - 현재 검색된 드라마와 유사 드라마들 검색 - by 1-blue
  useEffect(() => {
    if (!searchedDramas || searchedDramas.results.length === 0) return;

    dispatch(similarDrama({ dramaId: searchedDramas.results[0].id }));
  }, [searchedDramas, dispatch]);

  //2022/12/16 - 검색 토스트 처리 - by 1-blue
  const [title] = searchParams.getAll("title");
  useToastify({
    doneMessage: searchDramaDone && `"${title}"인 ${searchDramaDone}`,
    errorMessage: searchDramaError,
    callback: () => dispatch(movieActions.resetMessage()),
  });

  // 2022/12/16 - 검색 결과 중 메인으로 사용할 드라마 - by 1-blue
  const target = useMemo(() => {
    if (!searchedDramas || searchedDramas.results.length === 0) return null;

    return searchedDramas.results[0];
  }, [searchedDramas]);
  ...
  // 드라마 / 유사 드라마 검색중
  if (searchDramaLoading || similarDramaLoading) return <Loading.Movie />;
